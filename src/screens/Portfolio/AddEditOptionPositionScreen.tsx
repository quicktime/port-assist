// src/screens/Portfolio/AddEditOptionPositionScreen.tsx
...
type AddEditOptionPositionProps = {
  mode: 'add' | 'edit';
  position?: OptionPosition;
};

export default function AddEditOptionPositionScreen({ mode = 'add' }: AddEditOptionPositionProps) {
  const params = useLocalSearchParams();
  const isEditMode = mode === 'edit' || params.mode === 'edit';
  let editPosition: OptionPosition | undefined;
  
  if (isEditMode && params.position) {
    try {
      editPosition = JSON.parse(params.position as string) as OptionPosition;
    } catch (error) {
  ...
  // Form state
  const [underlying, setUnderlying] = useState(editPosition?.underlying || "");
  const [contractType, setContractType] = useState<'call' | 'put'>(editPosition?.contract_type || 'call');
  const [strikePrice, setStrikePrice] = useState(editPosition?.strike_price?.toString() || "");
  const [quantity, setQuantity] = useState(editPosition?.quantity?.toString() || "1");
  const [avgPrice, setAvgPrice] = useState(editPosition?.avg_price?.toString() || "");
  const [expirationDate, setExpirationDate] = useState(
    editPosition?.expiration_date ? new Date(editPosition.expiration_date) : new Date()
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <Appbar.Header>
        ...
        <Appbar.Content title={isEditMode ? "Edit Option Position" : "Add Option Position"} />
        <Appbar.Action 
      ...
      >
        <ScrollView style={styles.scrollView}>
          <View style={styles.formContainer}>
            ...
            <Text variant="bodyMedium" style={styles.label}>Underlying Symbol *</Text>
            ...
            <Text variant="bodyMedium" style={styles.label}>Contract Type *</Text>
            <SegmentedButtons
              value={contractType}
            ...
            <Text variant="bodyMedium" style={styles.label}>Strike Price *</Text>
            <TextInput
              mode="outlined"
              placeholder="e.g., 450"
              value={strikePrice}
            ...
            <Text variant="bodyMedium" style={styles.label}>Average Price Per Contract *</Text>
            <TextInput
              mode="outlined"
            ...
            <Button
              ...
              icon="content-save"
            >
              {loading ? "Saving..." : (isEditMode ? "Update Position" : "Add Position")}
            </Button>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
