...
/**
 * Screen for viewing detailed information about an option contract
 */
export default function DetailScreen() {
  const params = useLocalSearchParams();
  const paperTheme = useTheme();
  
  // Parse option data from params
  ...
  // If option data couldn't be parsed, return to previous screen
  if (!option) {
    router.back();
    return null;
  }
  ...
  return (
    <BaseScreen
      title={`${option.underlyingSymbol} ${option.optionType === 'call' ? 'Call' : 'Put'} $${option.strikePrice}`}
      showBackButton={true}
      onBack={() => router.back()}
    >
