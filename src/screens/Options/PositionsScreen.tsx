...
export default function OptionsPositionsScreen() {
  ...
  const handleAddPosition = () => {
    router.push('/(app)/options/add');
  };
  
  const handleEditPosition = (position: OptionPosition) => {
    router.push({
      pathname: '/(app)/options/edit',
  ...
  const handleViewDetails = (underlying: string) => {
    router.push(`/(app)/options-chain/${underlying}`);
  };
  ...
  if (loading) {
    return <BaseScreen title="Options Portfolio">
      <View style={commonStyles.loadingContainer}>
        <ActivityIndicator size="large" color={paperTheme.colors.primary} />
        <Text style={{ marginTop: 16 }}>Loading options positions...</Text>
      </View>
    </BaseScreen>;
  }
  
  return (
    <BaseScreen title="Options Portfolio">
        ...
        <FlatList
          ...
          ListEmptyComponent={() => (
            <EmptyState
              ...
              onButtonPress={handleAddPosition}
            />
          )}
        />
