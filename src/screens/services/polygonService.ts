...
// Helper function to call the polygon-proxy Edge Function
async function callPolygonProxy(endpoint: string, method: 'GET' | 'POST' = 'GET', body?: any): Promise<any> {
  try {
    ...
    console.error('Error calling Polygon proxy:', error);
    throw error;
  }
}
...
export const fetchMarketStatus = async (): Promise<string> => {
  try {
    ...
    try {
      ...
      const data = await callPolygonProxy(endpoint);
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        const response = await callPolygonProxy(endpoint);
...
const fetchOptionDetails = async (optionSymbol: string): Promise<any> => {
  try {
    ...
    try {
      ...
      const snapshotResponse = await callPolygonProxy(endpoint);
